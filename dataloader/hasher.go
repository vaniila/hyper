...
// LICENSE: https://github.com/cnf/structhash/blob/master/LICENSE
...
// function.
func toHash(c interface{}, version int) (string, error) {
	return fmt.Sprintf("v%d_%x", version, toMd5(c, version)), nil
}
...
func toDump(c interface{}, version int) []byte {
	return serialize(c, version)
}

// Md5 takes a data structure and returns its md5 hash.
// This is a shorthand for md5.Sum(toDump(c, version)).
func toMd5(c interface{}, version int) []byte {
	sum := md5.Sum(toDump(c, version))
	return sum[:]
}

// Sha1 takes a data structure and returns its sha1 hash.
// This is a shorthand for sha1.Sum(toDump(c, version)).
func toSha1(c interface{}, version int) []byte {
	sum := sha1.Sum(toDump(c, version))
	return sum[:]
}
